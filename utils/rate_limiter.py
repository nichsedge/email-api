...
class RateLimitStore:
        ...
        with self._lock:
            ...
            for key in list(self._api_key_limits.keys()):
                self._api_key_limits[key] = deque(
                    (timestamp, count)
                    for timestamp, count in self._api_key_limits[key]
            ...
            for ip in list(self._ip_limits.keys()):
                self._ip_limits[ip] = deque(
                    (timestamp, count)
                    for timestamp, count in self._ip_limits[ip]
                    if current_time - timestamp < window_seconds
                )
    ...
    def check_rate_limit(
        self, identifier: str, limit: int, window_seconds: int, is_api_key: bool = True
    ) -> bool:
        ...
        with self._lock:
            ...
            store[identifier] = deque(
                (timestamp, count)
                for timestamp, count in store[identifier]
                if current_time - timestamp < window_seconds
            )
...
class APIKeyRateLimiter:
    ...
    def check_rate_limit(
        self, api_key_id: str, rate_limit_per_minute: int, rate_limit_per_hour: int
    ) -> tuple[bool, int, int]:
        ...
        with self._lock:
            ...
            hour_limit = rate_limit_store.check_rate_limit(
                f"api_key:{api_key_id}:hour", rate_limit_per_hour, 3600, is_api_key=True
            )
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if api_key:
        ...
        allowed, remaining_minute, remaining_hour = (
            api_key_rate_limiter.check_rate_limit(
                api_key.key_id,
...
def check_api_key_rate_limit(api_key):
    ...
    allowed, remaining_minute, remaining_hour = api_key_rate_limiter.check_rate_limit(
        api_key.key_id, api_key.rate_limit_per_minute, api_key.rate_limit_per_hour
    )
