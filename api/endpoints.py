...
async def create_api_key(
    api_key_data: APIKeyCreate, request: Request, db: Session = Depends(get_db)
):
...
async def update_api_key(
    ...
    try:
        ...
        if (
            not check_scope(current_api_key, "admin")
            and current_api_key.key_id != key_id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions"
            )
        ...
        return SuccessResponse(status="success", message="API key updated successfully")
...
async def delete_api_key(
    ...
    try:
        ...
        return SuccessResponse(status="success", message="API key deleted successfully")
...
async def send_email(
    ...
    try:
        ...
        return SuccessResponse(
            status=result["status"], message=result["message"], data=result
        )
...
async def read_emails(
    ...
    try:
        ...
        return EmailListResponse(count=len(emails), emails=emails)
...
async def mark_email_as_read(
    ...
    try:
        ...
        result = secure_email_client.mark_email_as_read(
            email_credentials=email_credentials, email_id=id
        )
...
async def mark_emails_as_read_batch(
    ...
    try:
        ...
        result = secure_email_client.mark_emails_as_read_batch(
            email_credentials=email_credentials, email_ids=batch_request.email_ids
        )
