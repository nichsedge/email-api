...
def get_filtered_unread_emails(
    filter_by="today", start_date=None, end_date=None, mark_as_read=False
):
        ...
        with imaplib.IMAP4_SSL(imap_server, imap_port) as mail:
            ...
            if filter_by == "today":
                ...
                if not start_date or not end_date:
                    raise ValueError(
                        "start_date and end_date must be provided for 'date_range'"
                    )
                search_criteria += [
                    "SENTSINCE",
                ...
                raise ValueError(
                    "Invalid filter_by option. Use 'today', 'date_range', or 'all'."
                )
                        ...
                        if msg.is_multipart():
                            for subpart in msg.walk():
                                if (
                                    ...
                                    and "attachment"
                                    not in str(subpart.get("Content-Disposition"))
                                ):
                                    try:
