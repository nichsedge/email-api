...
class APIKeyCreate(BaseModel):
    ...
    scopes: List[EmailScope] = Field(
        default=["read"], description="List of allowed scopes"
    )
    rate_limit_per_minute: int = Field(
        default=60, ge=1, le=1000, description="Requests per minute limit"
    )
    rate_limit_per_hour: int = Field(
        default=1000, ge=1, le=10000, description="Requests per hour limit"
    )
...
class EmailBatchRequest(BaseModel):
    ...
    email_ids: List[str] = Field(
        ..., min_items=1, max_items=100, description="List of email IDs"
    )
...
class EmailFilterRequest(BaseModel):
    ...
    mark_as_read: bool = Field(
        default=False, description="Mark emails as read after retrieval"
    )
    ...
    def validate_dates(cls, v, values):
        ...
        if values.get("filter_by") == FilterBy.DATE_RANGE:
            if v is None:
                raise ValueError(
                    "Date range filter requires both start_date and end_date"
                )
        return v
